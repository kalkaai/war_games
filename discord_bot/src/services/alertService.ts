...
import { getAllUsers, listAccounts } from './accountService'
...
export async function runAlertCheck(client: Client): Promise<void> {
  ...
  for (const user of users) {
    ...
    for (const account of accounts) {
      for (const threshold of user.alertThresholds) {
        const timers = await getTimersDueForAlert(user.discordId, account.name, threshold)
        ...
        for (const timer of timers) {
          try {
            ...
            // Calculate next reset info for recurring timers
            ...
            // Auto-reset recurring timers after expiry notification
            ...
            console.warn(`Could not DM user ${user.discordId}:`, err)
          }
        }
      }
    }
